...
function generateFallbackAnswer(userPrompt: string): string {
  const lastMsg = userPrompt.toLowerCase();

  if (lastMsg.includes("project") || lastMsg.includes("built")) {
    return `Pranav has built several high-impact software & AI projects across two main tracks:
...
- [🤖 AI/ML Resume](/resumes/Pranav_Kakde_AIML_GenAI_Resume.pdf)`;
  ...
  if (lastMsg.includes("spring boot") || lastMsg.includes("java") || lastMsg.includes("backend")) {
    return `Yes! Pranav specializes heavily in **Java & Spring Boot Development**. 
...
Feel free to check out his [📄 Java/Backend Resume](/resumes/Pranav_Kakde_Java_Backend_Resume.pdf) or contact him at **kakdepranav993@gmail.com**.`;
  ...
  if (lastMsg.includes("resume") || lastMsg.includes("contact") || lastMsg.includes("email") || lastMsg.includes("education")) {
    return `Here are Pranav's background, education, and contact details:
...
- [🤖 Download AI / ML & GenAI Resume](/resumes/Pranav_Kakde_AIML_GenAI_Resume.pdf)`;
  ...
  return `Hello! I am **Pranav's AI Assistant**. I can help you explore Pranav Kakde's engineering qualifications, projects, skills, education, and resumes.
  ...
  try {
    ...
    if (apiKey) {
      try {
        ...
        const stream = new ReadableStream({
          async start(controller) {
            try {
              ...
              const fallback = generateFallbackAnswer(latestUserMessage);
        ...
        console.warn("Gemini API call failed, serving grounded stream fallback:", geminiErr);
      }
    }

    // Stream fallback generator if no API key or API call failed
    const fallbackText = generateFallbackAnswer(latestUserMessage);
    const encoder = new TextEncoder();
    const stream = new ReadableStream({
      async start(controller) {
